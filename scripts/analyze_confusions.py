...
def _confusions_from_record(record: dict, iou_threshold: float) -> list[dict]:
    ...
    if isinstance(pre, list) and pre:
        ...
        filtered = [c for c in pre if isinstance(c, dict) and c.get("iou", 0.0) >= iou_threshold]
        for c in filtered:
...
def compute_stats(jsonl_path: Path, iou_threshold: float) -> dict:
    ...
    with open(jsonl_path, "r") as fh:
        for line in fh:
            ...
            for c in confusions:
                ...
                if len(stats["example_frame_ids"]) < 5 and frame_id not in stats["example_frame_ids"]:
    ...
    for key, stats in pair_stats.items():
        ...
        results.append({
            ...
            "avg_score_a": round(sum(sa_vals) / len(sa_vals), 4) if sa_vals else 0.0,
            "avg_score_b": round(sum(sb_vals) / len(sb_vals), 4) if sb_vals else 0.0,
            "example_frame_ids": stats["example_frame_ids"],
            "suggestion": _suggest(
                stats["prompt_a"], stats["prompt_b"],
                rate, sum(iou_vals) / len(iou_vals) if iou_vals else 0.0,
    ...
    return {
        ...
        "confusion_rate": round(frames_with_confusion / total_frames, 4) if total_frames > 0 else 0.0,
    ...
    if not stats["prompt_pairs"]:
        ...
        for pair in stats["prompt_pairs"]:
            ...
            if len(suggestion) > 77:
                for chunk in [suggestion[i:i+77] for i in range(77, len(suggestion), 77)]:
...
def main() -> int:
    ...
    parser.add_argument(
        "--iou-threshold", type=float, default=0.95,
    ...
    stats["prompt_pairs"] = [
        p for p in stats["prompt_pairs"] if p["frames_with_confusion"] >= args.min_frames
    ]
